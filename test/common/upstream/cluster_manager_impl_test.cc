...
public:
  ...
  Http::ConnectionPool::InstancePtr allocateConnPool(Event::Dispatcher&, HostConstSharedPtr host,
                                                     ResourcePriority, uint32_t so_mark,
                                                     enum Http::Protocol) override {
    UNREFERENCED_PARAMETER(so_mark);
...
TEST_F(ClusterManagerImplTest, UnknownCluster) {
  ...
  EXPECT_EQ(nullptr,
            cluster_manager_->httpConnPoolForCluster("hello", ResourcePriority::Default,
